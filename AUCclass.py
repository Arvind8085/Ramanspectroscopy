...
class NitroPhenolAnalysis:
    def __init__(self, file_path2):
        self.file_path = file_path2
        self.analysis_df2 = None

    def load_and_prepare_data2(self):
        """Loads Excel file and prepares analysis DataFrame."""
        self.analysis_df2 = pd.read_excel(self.file_path, sheet_name='Analysis')
        self.analysis_df2['Wavenumber 1'] = self.analysis_df2['Wavenumber 1'].round()
        self.analysis_df2['Wavenumber 2'] = self.analysis_df2['Wavenumber 2'].round()
        self.analysis_df2.sort_values(by=['Wavenumber 1', 'Wavenumber 2'], inplace=True)
        self.analysis_df2.sort_values(by='Sum of Mean Heights', ascending=False, inplace=True)
        self.analysis_df2['Priority'] = range(1, len(self.analysis_df2) + 1)

    @staticmethod
    def find_best_r22(concentrations, mean_AUC):
        ...
        for i in range(len(concentrations) - 2):
            ...
            sub_heights = mean_AUC[i:]
            slope, intercept, r_value, _, _ = linregress(sub_conc, sub_heights)
            print(f"Subset {i+1}: Concentrations {sub_conc}, Mean AUC {sub_heights}, R²={r_value**2:.6f}")
    ...
    def plot_best_fit_ratio_transformed2(self, cluster1, cluster2, priority, label):
        """Returns a plot, regression stats, and subset info for a cluster ratio."""
        ratio_df = self.analysis_df2[
            (self.analysis_df2['Cluster 1'] == cluster1) & 
            (self.analysis_df2['Cluster 2'] == cluster2)
        ]
        if ratio_df.empty:
            raise ValueError(f"No data for Cluster {cluster1}/{cluster2}")

        valid_ratio_df = ratio_df.dropna(subset=['Mean AUC Ratio', 'Concentration'])
        valid_ratio_df = valid_ratio_df[valid_ratio_df['Mean AUC Ratio'] > 0]
        ...
        mean_peak_heights = valid_ratio_df['Mean AUC Ratio'].values
        std_peak_heights = valid_ratio_df['Std AUC Ratio'].values

        # Find best fit
        best_r2, best_slope, best_intercept, best_subset = self.find_best_r22(log_concentrations, mean_peak_heights)
        ...
        for i in range(len(log_concentrations) - 2):
            ...
            subset_details.append(
                f"Subset {i + 1}: Concentrations {sub_conc}, Mean AUC Heights {sub_heights}, R²={r_value**2:.6f}"
            )
    ...
    def generate_plots2(self, top_n=60):
        """Generates plots for top N cluster ratios based on sum of peak heights."""
        unique_ratios = self.analysis_df2[['Cluster 1', 'Cluster 2', 'Sum of Mean Heights', 'Priority']].drop_duplicates()
        ...
        for _, row in top_ratios.iterrows():
            ...
            try:
                fig, best_r2, best_slope, best_intercept, subset_details = self.plot_best_fit_ratio_transformed2(
...
if __name__ == "__main__":
    ...
    plotter = NitroPhenolAnalysis(file_path)
    plotter.load_and_prepare_data2()
    plotter.generate_plots2(top_n=60)
